...
yr_regex = re.compile("A([0-9]{4})([0-9]{2})")
...
input_file_dir = os.path.join(dir_root, f"{file_var}_monthly_005d_V1")
input_year_files = Path(input_file_dir).rglob(f"{file_var}_Monthly_005d.*.nc")
...
outfile = os.path.join(dir_root, "limits", f"{var_name}_monthly_limits.csv")

with open(outfile, "w", buffering=1024) as outf:

    # Loop over the files
    for (year, mn_idx), this_file in year_files:
        ...
        try:
            ...
            outf.write(f"{year},{mn_idx},NA,NA,NA,NA # {err_string}\n")
            continue
        ...
        # Report limits
        outf.write(
            f"{year},{mn_idx},{na_count},{inf_count},{float(mat.min())},{float(mat.max())}\n"
        )
