...
class ProcessData():
    ...
    * provide the get_data method to extract the monthly data for a given year.
    ...
    def __init__(self, decade_files):
        ...
        self.monthly_data['vpd'] = convert_vp_to_vpd(
...
for cru_decade in cru_data_by_decade:
    ...
    for year in cru_decadal_data.years:
        ...
        export_data = xarray.Dataset(
            data_vars=dict(
                potential_gpp_c3=(["month", "lat", "lon"], pmodel_c3.gpp.astype(np.float32)),
                potential_gpp_c4=(["month", "lat", "lon"], pmodel_c4.gpp.astype(np.float32)),
                mean_monthly_water_stress=(
                    ["month", "lat", "lon"], 
                    monthly_mean_water_stress['soilmstress_mengoli'].to_numpy().astype(np.float32)
                ),
            ),
            coords={
                "month": time_coords,
